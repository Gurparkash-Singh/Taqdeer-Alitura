...
export async function load({ params, url }) {
	...
	if (!token) {
		await dbFunctions.setError('password reset', 400, `${email} tried an invalid token`);
		return error(404, {
			invalid: true,
...
export const actions = {
	submit: async ({ locals, cookies, request }) => {
		...
		if (!email || !token) {
			await dbFunctions.setError('password reset', 400, `${email} had no email or token`);
			return fail(400, {
				invalid: true,
		...
		if (!password || !confirmPassword) {
			await dbFunctions.setError('password reset', 400, `${email} left a field empty`);
			return fail(400, {
				invalid: true,
		...
		if (!profileEditor.passwordsMatch(password, confirmPassword)) {
			await dbFunctions.setError('password reset', 400, `${email} passwords don't match`);
			return fail(400, {
				invalid: true,
		...
		if (!profileEditor.validNewPassword) {
			await dbFunctions.setError('password reset', 400, `${email} invalid password`);
			return fail(400, {
				invalid: true,
				message: message,
				token,
