...
export const actions = {
	register: async ({ cookies, request }) => {
		...
		if (!name || !email || !password || !confirmPassword) {
			await dbFunctions.setError('signup', 400, `${email} left a field empty`);
			return fail(400, {
				invalid: true,
		...
		if (!profileEditor.passwordsMatch(confirmPassword, password)) {
			await dbFunctions.setError('signup', 400, `${email} passwords don't match`);
			return fail(400, {
				invalid: true,
		...
		if (!profileEditor.validNewPassword(password)) {
			await dbFunctions.setError('signup', 400, `${email} typed an invalid password`);
			return fail(400, {
				invalid: true,
				message: message,
				name: name,
		...
		if (invalidEmail) {
			await dbFunctions.setError('signup', 400, `${email} is invalid\nError: ${invalidEmail}`);
			return fail(400, {
				invalid: true,
