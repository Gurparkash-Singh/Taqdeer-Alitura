<script>
	...
	let price = $derived(floatPrice.toLocaleString(numberFormat.area, numberFormat.style));
...
<section id="product">
	<div id="image-carousel">
		...
		{#each data.images as image}
			...
			>
				<picture>
					<source media="(min-width:640px)" srcset={image.large_image} />
					<source media="(min-width:320px)" srcset={image.medium_image} />
					<img src={`${image.image_link}`} alt={image.alt_desc} />
				</picture>
			</button>
		{/each}
		<button aria-label="next image" onclick={nextImage} class:oneImage={data.images.length <= 1}>
			<svg
				width="20"
				height="30"
				viewBox="0 0 34 60"
	...
	<section class="product-modifier">
		{#each product_variations as variation}
			<div class="options-selector" style:flex-basis="100%">
				{#each data.product_variation_options as option}
					{#if variation.id === option.variation_id}
						...
						{:else}
							<button
								class:selected={selection.product_modifiers[option.variation_id] ===
									option.option_id}
								onclick={() => {
									selection.update_modifiers(option.variation_id, option.option_id);
								}}
							>
								{option.value}
							</button>
		...
		{/each}
		<div id="size-selector" style:grid-template-columns={`repeat(${templateColumns + 1}, 1fr)`}>
		...
		<form method="POST" action="?/add" id="cart-button">
			...
			/>
			<button class:disabled={selection.quantity === 0} disabled={selection.quantity === 0}>
				Add to cart
			</button>
		</form>
	</section>
	...
	<section class="collapsable" id="info" class:open-section={openInfo}>
	...
	<section class="collapsable" id="size" class:open-section={openSize}>
		...
		<table style:display={sizeChartUnit === 0 ? 'block' : 'none'}>
			<thead>
				<tr>
					<th>Size</th>
			...
			<tbody>
				{#each data.size_chart_values as components_array}
					<tr>
						<th>
							{components_array.length > 0 ? components_array[0].size : null}
						</th>
		...
		<table style:display={sizeChartUnit === 1 ? 'block' : 'none'}>
			<thead>
				<tr>
					<th>Size</th>
					{#each data.size_chart_components as component}
	...
	<section class="collapsable" id="return" class:open-section={openReturn}>
		<p>
			For returns or exchanges, please contact support.
			<br />
			Returns and exchanges must be requested within 1 week of purchase.
			<br />
