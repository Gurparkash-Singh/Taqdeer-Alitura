<script>
	...
	async function uploadFile(file) {
		...
		try {
			...
			res = await fetch(`${url}/upload/${data.product.product_id}`, {
				method: 'POST',
				body: formData
			});
		} catch (error) {
	...
	async function deleteImage(image_id) {
		...
		try {
			res = await fetch(`${url}/delete/${image_id}/${data.product.product_id}`, {
				method: 'POST'
			});
		} catch (error) {
			console.log(error);
...
<div id="upload-modal" class:enabled={numUploads < totalUploads}>
	<h1>{uploadText}</h1>
</div>
...
<section>
	{#each data.images as image}
		...
		>
			...
			{/if}
			<img src={`${image.small_image}`} alt={image.alt_desc} />
		</button>
	{/each}

	<button aria-label="Add Product" class="product-links" id="add-button" onclick={onSelectFiles}>
		<svg
			fill="#000000"
			height="800px"
			width="800px"
...
<form action="?/reorder" method="POST" id="update-form">
