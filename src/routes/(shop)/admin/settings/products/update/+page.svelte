...
<section id="shop">
	<header>
		...
		<div>
			...
			<ul class:open_menu={open_sort_menu} id="sort-menu">
				...
				<li class:selected={selected_price == 1} class="lastListElement">
					<button
						onclick={() => {
							selected_price = 1;
							sortProductsByPrice();
		...
		<div>
			...
			<ul class:open_menu={open_collections_menu}>
				{#each collections as collection}
					<li
						class:selected={selected_collection == collection.collection_id}
						class:lastListElement={collection === collections[collections.length - 1]}
					>
						<button
							onclick={() => {
	...
	{#each filteredProducts as product (product.product_id)}
		<a href="/admin/{product.product_id}" class="product-links">
			<SettingsCard {product} images={data.images} />
		</a>
	{/each}

	<a href="./update/add" aria-label="Add Product" class="product-links" id="add-button">
		<svg
			fill="#000000"
			height="800px"
			width="800px"
