<script>
	...
	const payment_images = import.meta.glob('$lib/images/payment_logos/*.png', {
		eager: true,
		query: {
			enhanced: true
		}
...
<section id="cart">
	<section id="cart-items-holder" class:no-product={data.cart_items.length == 0}>
		<header>
			<ul>
				<li>Item name</li>
				<li>Size</li>
		...
		{#if data.cart_items.length > 0}
			<form action="?/clear" method="POST" id="clear-button">
				<button formaction="?/clear" formmethod="POST" type="submit"> clear cart </button>
			</form>
		{/if}
	</section>
	...
	<section id="checkout-form">
		<a href="/cart/delivery" class:disable-submit={data.cart_items.length == 0}>
			Checkout
			<svg
				width="15"
				height="15"
		...
		<ul>
			{#each Object.entries(payment_images) as [_path, module]}
				<li>
					<enhanced:img src={module.default} alt="payment options" class="payment-logos" />
				</li>
			{/each}
		</ul>
	</section>
	<section id="checkout-area">
		<section id="extra-info">
			...
			<section class="collapsable" class:open-section={openInstallments}>
				<p>Buy now and pay in four zero interest installments billed monthly with tabby.</p>
			</section>
			...
			<section class="collapsable" class:open-section={openCoupons}>
				<p>
					Enter coupon code, or view discounts associated with your account. This will be applied to
					the subtotal
				</p>
