...
export async function load({ locals, params, url }) {
	...
	if (submitError == 'empty') {
		await dbFunctions.setError('account verification', 400, `${locals.user.email} Empty fields`);
		return {
			service: service,
	...
	if (submitError == 'token') {
		await dbFunctions.setError('account verification', 400, `${locals.user.email} invalid token`);
		return {
			service: service,
	...
	if (phone) {
		...
		try {
			await client.verify.v2.services(TWILIO_SERVICE_CODE).verifications.create({
				to: phone.number,
				channel: 'sms'
			});
		} catch (error) {
