...
export async function load({ cookies, params, url }) {
	...
	try {
		...
		await dbFunctions.setCriticalError('orders', 500, JSON.stringify(axiosError, null, 2));
	...
	const paymentInsertResult = await dbFunctions.addPaymentDetails(amount, provider, payment_status);
	...
	if (!order) {
		dbFunctions.setCriticalError('orders', 500, `order not found for order: ${order_id}`);
		error(500);
	}
	...
	if (!address) {
		dbFunctions.setCriticalError('orders', 500, `address not found for order: ${order_id}`);
		error(500);
	}
	...
	if (!order.tracking_id) {
		...
		for (let i = 0; i < order_items.length; i++) {
			num_items +=
				parseInt(order_items[i].quantity) * parseFloat(order_items[i].default_box_volume);
			customs_value += parseInt(order_items[i].quantity) * parseFloat(order_items[i].price);
			weight += parseFloat(order_items[i].weight) * parseInt(order_items[i].quantity);
		...
		if (aramexResult.HasErrors) {
			...
			await dbFunctions.setCriticalError('orders', 500, `${JSON.stringify(errors, null, 2)}}`);
