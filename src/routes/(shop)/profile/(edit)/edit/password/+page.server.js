...
export const actions = {
	submit: async ({ locals, cookies, request }) => {
		...
		const emptyFields = profileEditor.emptyFields([oldPassword, password, confirmPassword]);

		if (!oldPassword || !password || !confirmPassword) {
			await dbFunctions.setError('password change', 400, `${locals.user.email} left empty fields`);
			return fail(400, {
				invalid: true,
