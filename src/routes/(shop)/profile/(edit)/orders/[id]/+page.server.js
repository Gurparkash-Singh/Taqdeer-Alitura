...
export const actions = {
	submit: async ({ locals, cookies, request }) => {
		...
		for (let i = 0; i < order_items.length; i++) {
			...
			if (item.quantity < 0 || item.quantity > selected_size.quantity || item.quantity > 5) {
				product_errors = true;
			}
			...
			if (!current_item && item.quantity > 0) {
				await dbFunctions.addToCart(shopping_session, item.product_id, item.size_id, item.quantity);
				break;
			}

			await dbFunctions.updateCart(shopping_session, item.product_id, item.size_id, item.quantity);
		}
